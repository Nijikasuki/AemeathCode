...
# /about 的字符画是 90 列(占 93,含左右 padding)。窄于此就不画 —— 画不能折行,
# 裁掉半张脸不如没有。余下的宽度给文字栏,它是弹性的、不够会折行。
# 125 = 画 93 + 文字栏 32。32 是让 `model      <模型名>` 这行对常见模型名不折行的下限;
# 更窄仍然能显示(信息不丢,只是折得难看),所以这个数是**观感**下限,不是功能下限。
#
# 换算成终端列数:content-body 宽 = 列数 - 72
#   66 = #side 34 + #aside 32(固定) · 4 = #content 的边框 2 + padding 2
#   2  = #content-body 自己的 padding
# 所以 125 对应终端 ≥197 列。
ABOUT_ART_MIN_WIDTH = 125
